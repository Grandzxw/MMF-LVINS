...
public: 
    ...
    // User-side API  自定义空间
    void makeAndSaveSpatialcontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down );
    std::tuple<int, float, float> detectLoopClosureID( void ); // int: nearest node index, float: relative yaw  

    double simDirect ( MatrixXd &_vec1, MatrixXd &_vec2 );
    ...
    //get spatial decs
    std::tuple<vector<vector<vector<int>>>,vector<vector<vector<double>>>,vector<vector<vector<double>>>> makeSpatialState(pcl::PointCloud<SCPointType>  &cloud);
    //generate spatial descriptors from state of each bins
    Eigen::MatrixXd getSpatialDesc(std::tuple<vector<vector<vector<int>>>,vector<vector<vector<double>>>,vector<vector<vector<double>>>>& _istate);
    MatrixXd makeRingFingerprint(vector<vector<vector<double>>>& _intensity_state);  //density密度
    //MatrixXd makeRingFingerprint(vector<vector<vector<int>>>& _density_state);



    std::pair<double, int> calculateDistanceBtnPC (pcl::PointCloud<SCPointType> &cloud1, pcl::PointCloud<SCPointType> &cloud2 );
...
public:
    ...
    std::vector<Eigen::MatrixXd> spatial_desc;
